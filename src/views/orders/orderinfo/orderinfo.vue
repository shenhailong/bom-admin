  ...
  data() {
    return {
      ...
      columns: [
        ...
        {prop: 'taxinclud', label: '税率', width: '160'},
      ...
      act_btn: [
        { name: '修改', code: 'UPDATE', when: {'billState': [0]}, action: updateProductOrder },
        { name: '删除', code: 'DELETE', when: {'billState': [0]}, action: deleteProductOrderByPks },
        { name: '提交订单', code: 'COMMIT', when: {'billState': [0]}, action: updateProductOrder },
        { name: '查看', code: 'LOOKUP', when: {'billState': [30, 31]} }
      ],
      replace: [
