      ...
      <el-col :span="24">
        <el-row>
          ...
          <el-col :span="11" class="icon" :offset="1">
            ...
            <i class="el-icon-folder-opened"><span>文件</span></i>
...
import {
  selectAllMaterielWareHouseAsPage,
  updateMaterielWareHouse,
} from "@/api/orderCompMage/OrderWarehouse";
  ...
  methods: {
    ...
    queryViewListDataByWhere(whereParam) {
      ...
      if (whereParam == "ALL") {
        ...
        if (queryParam.billState == null) {
          const actList = this.act_btn;
          for (var i = 0; i < actList.length; i++) {
            if (actList[i].when) {
              actList[i].when = { dis: [1] };
            }
          }
        }
      } else {
        queryParam.billState = this.viewDefaultBillState;
        if (queryParam.billState == "2") {
          const actList = this.act_btn;
          for (var i = 0; i < actList.length; i++) {
            if (actList[i].when) {
              actList[i].when = { dis: [0] };
            }
          }
        } else {
          const actList = this.act_btn;
