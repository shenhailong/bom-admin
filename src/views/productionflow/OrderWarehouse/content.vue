<template>
  <el-dialog
    ...
    width="45%"
  >
    <el-form :inline="true" ref="dataForm" :model="temp" label-position="left">
      <el-form-item
        label="文件名称"
        prop="fileName"
        class="file"
      ...
      <el-form-item label="文件全路径名称" prop="fileFullname" class="loader">
        <el-input v-model="temp.fileFullname"></el-input>
      </el-form-item>
      <el-form-item label="单据状态" prop="billState" label-width="110px">
        <el-select
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="handleSaveWarehouseFile"
        >保存上传</el-button
      >
...
export default {
  ...
  computed: {
    ...mapGetters(["columns", "name", "userid"]),
  },
  data() {
    return {
            ...
            .then(() => {
              ...
              updateMaterielWareHouse(saveParam).then((responseT) => {
                if (responseT && responseT.success) {
                  ...
                  this.$message({
                    message: `操作失败:${responseT.message}`,
                    type: "error",
                  });
                }
              });
