<template>
  <div>
      ...
      <el-col :span="24">
        <el-row>
          ...
          <el-col :span="11" class="icon">
            ...
            <i class="el-icon-s-home"><span>物料入库</span></i>
    ...
    <!--物料入库子表展示界面Dialog-->
    <ProductionFlowOrderRepertoryContent
      :replace="replace"
...
import ProductionFlowOrderRepertoryContent from "@/views/productionflow/OrderRepertory/subinfo/subinfo";
...
import {
  selectAllMaterielPutinWareAsPage,
  updateMaterielPutinWare,
} from "@/api/orderCompMage/OrderRepertory";
import { materielPutinProccess } from "@/api/orderCompMage/orderCompMage";
import {
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { prop: "scheduledtime", label: "物料需求日期", width: "150" },
        { prop: "pkOmgPutinWare", label: "物料入库主键", primary: true },
  ...
  methods: {
    ...
    dingding() {
      this.onButtonClick({btnCode:'COMMITDINGDING'})
    },
      drawers() {
      //消息中心的调用
    ...
    onButtonClick(param, btnCode, unexParam) {
      ...
      if (param.btnCode === "OVERREPERTORY") {
        // 全部入库
        if (param.bindData.pkProductOrderB) {
          ...
          this.$confirm("是否操作该项?", "提示", {
            ...
            .then(() => {
              ...
              materielPutinProccess(mrpParam).then((response) => {
