<template>
  <el-dialog
    ...
    width="90%"
  >
    <!-- <el-button size="mini" type="primary" @click="onButtonClick('ADDSUBDATA', null)">录入</el-button> -->
    <el-table
    ...
    >
      ...
      <el-table-column prop="estimatedEndTime" label="预计结束时间" width="180">
      </el-table-column>
      <el-table-column prop="estimatedNumber" label="预计完工数量" width="180">
      </el-table-column>
      <el-table-column prop="processPrice" label="未税单价" width="180">
      </el-table-column>
      <el-table-column prop="processTotalPrice" label="未税合计" width="180">
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <!-- <el-form-item label="排序" class="paixu">
          <el-input-number
            v-model="subInfoContent.sort"
            :min="1"
            :max="1000"
            label="排序"
          ></el-input-number>
        ...
        <el-form-item label="生产车间">
          <el-select
            v-model="subInfoContent.productionWorkshop"
            clearable
            placeholder="请选择"
          >
            <el-option
              v-for="item in productionWorkOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
        ...
        <!-- <el-form-item v-if="typethis == 1" label="SMT焊点">
          <el-input v-model="subInfoContent.pkOmgOrderPlan"></el-input>
        </el-form-item>
        <el-form-item v-if="typethis == 2" label="DIP焊点">
          <el-input v-model="subInfoContent.pkOmgOrderPlan"></el-input>
        </el-form-item>
        <el-form-item v-if="typethis == 3" label="三防面积">
          <el-input v-model="subInfoContent.pkOmgOrderPlan"></el-input>
        </el-form-item>
        <el-form-item v-if="typethis == 4" label="组装工时">
          <el-input v-model="subInfoContent.pkOmgOrderPlan"></el-input>
        </el-form-item> -->
        <!-- typethis -->
        ...
        <el-form-item label="创建时间">
          <el-input
            v-model="subInfoContent.creationtime"
            :disabled="true"
          ></el-input>
        </el-form-item>
        ...
        <!-- <el-form-item label="订单工序总价">
          <el-input v-model="orderallprice"></el-input>
        </el-form-item> -->
        <el-form-item label="未税单价">
          <el-input
            v-model="subInfoContent.processPrice"
            @input="sumPrice"
            placeholder="请输入未税单价"
          ></el-input>
        </el-form-item>
        <el-form-item label="未税合计">
          <el-input
            v-model="subInfoContent.processTotalPrice"
            :disabled="true"
          ></el-input>
        </el-form-item>
...
import {
  selectAllOrderSubDataAsPage,
  saveOrderPlanSubData,
  ...
  data() {
    return {
      ...
      productionWorkOptions: [
        //车间选择
        { value: "创元成业", label: "创元成业" },
        { value: "香河分厂", label: "香河分厂" },
        { value: "张家口分厂", label: "张家口分厂" },
      ],
      billStateOptions: [
        { value: "0", label: "自由态" },
        { value: "1", label: "进行中" },
      ...
      allprice: 0, //工序单价
      aprice: 0, //工序总价
      orderallprice: 0, //订单工序总价
    };
  },
  watch: {
    // aprice() {
    //   // console.log(this.subInfoContent.aprice );
    //   this.allprice = 0;
    //   this.allprice =
    //     Number(this.aprice) * Number(this.subInfoContent.pkOmgOrderPlan);
    //   this.orderallprice =
    //     Number(this.allprice) * Number(this.subInfoContent.estimatedNumber);
    // },
    visiable() {
  ...
  methods: {
    ...
    onButtonClick(btnCode, rowData) {
      ...
      if (btnCode === "updatePlan") {
        // console.log(rowData);
        if (rowData.procedureName == "SMT") {
          this.typethis = 1;
        } else if (rowData.procedureName == "DIP") {
          this.typethis = 2;
        } else if (rowData.procedureName == "三防") {
          this.typethis = 3;
        } else if (rowData.procedureName == "组装") {
          this.typethis = 4;
        }
        this.subInfoContent = rowData;
        // this.subInfoContent.aprice = 0
        // this.subInfoContent.allprice =0
        this.subInfoContentVisiable = true;
      }
      ...
      if (btnCode === "SAVESUBDATA") {
        ...
        // debugger
        if (this.subInfoContent.estimatedStartTime) {
          this.subInfoContent.estimatedStartTime = parseTime(
            this.subInfoContent.estimatedStartTime
          );
          // this.subInfoContent.estimatedStartTime = parseTime(this.subInfoContent.estimatedStartTime, '{y}-{m}-{d}')
          // console.log(this.subInfoContent.estimatedStartTime);
        }
        if (this.subInfoContent.estimatedEndTime) {
          this.subInfoContent.estimatedEndTime = parseTime(
            this.subInfoContent.estimatedEndTime
          );
          // console.log(this.subInfoContent.estimatedEndTime);
        }
        if (this.subInfoContent.actualStartTime) {
          this.subInfoContent.actualStartTime = parseTime(
            this.subInfoContent.actualStartTime
          );
          // console.log(this.subInfoContent.actualStartTime);
        }
        if (this.subInfoContent.actualEndTime) {
          this.subInfoContent.actualEndTime = parseTime(
            this.subInfoContent.actualEndTime
          );
          // console.log(this.subInfoContent.actualEndTime);
        }
    ...
    //进行总价计算
    sumPrice() {
      this.subInfoContent.processTotalPrice =
        this.subInfoContent.processPrice * this.subInfoContent.estimatedNumber;
      this.subInfoContent.processTotalPrice = this.subInfoContent.processTotalPrice.toFixed(
        4
      );
    },
    // 子表录入界面属性描述
