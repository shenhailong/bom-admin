<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <el-row>
          ...
          <el-col :span="8" class="button" :offset="1">
            <!-- 按钮状态 -->
          ...
          <el-col :span="11" class="icon">
            ...
            <i class="el-icon-check"><span>出库</span></i>
            <i class="el-icon-finished"><span>入库完成</span></i>
    ...
    <!--产品入库详情Dialog-->
    <ProductionFlowOrderIncomeContent
      :replace="replace"
...
import ProductionFlowOrderIncomeContent from "@/views/productionflow/OrderIncome/subinfo/subinfo";
...
import {
  ...
  insertOmgPrduFinish,
  deleteOmgPrduFinisByPks,
  updateOmgPrduFinis,
  selectAllPrduFinisSubDataAsPage,
} from "@/api/orderCompMage/OrderIncome";
import { putInStartPutOut } from "@/api/orderCompMage/orderCompMage";
import {
...
export default {
  name: "ProductionFlowOrderIncomeView",
  components: {
    Table,
    CommitDialog,
    ProductionFlowOrderIncomeContent,
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: "pkOmgPrduFinishPutin", label: "成品入库主键", primary: true },
      ...
      select: {
        select: selectAllOmgPrduFinishAsPage,
  ...
  methods: {
    ...
    onButtonClick(param, btnCode, unexParam) {
        ...
        if (param.bindData.pkProductOrderB) {
          ...
          this.$confirm("是否确认出库?", "提示", {
            ...
            .then(() => {
              ...
              putInStartPutOut(beginParam).then((response) => {
      ...
      if (param.btnCode === "OVERINCOME") {
        // 入库完成
        if(param.bindData.finishNum >= param.bindData.constractProductNum){
        if (param.bindData.pkOmgPrduFinishPutin) {
          let overParam = {
            pkOmgPrduFinishPutin: param.bindData.pkOmgPrduFinishPutin,
            billState: "2",
            pkProductOrderB: param.bindData.pkProductOrderB,
          };
          ...
          this.$confirm("是否确认入库?", "提示", {
            ...
            .then(() => {
              ...
              updateOmgPrduFinis(overParam).then((response) => {
                if (response && response.success) {
                   this.refreshTableList(this.selectedConstractData);
                    this.label(this.ressl)
              // location.reload();
