<template>
  ...
  >
    ...
    >
      ...
      <el-button
        type="primary"
        :disabled="captchaBtn.disable"
        :class="{ codeGeting: captchaBtn.isGeting }"
        @click="getVerifyCode"
        >{{ captchaBtn.getCode }}</el-button
      >
    </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        registerUsername: [{ validator: checkPhone, trigger: "blur" }],
        registerPassword: [
          ...
          if (this.captchaBtn.shortMsgCaptcha === this.temp.captcha) {
            this.$confirm("是否确认注册 ?", "提示", {
              ...
              .then(() => {
                ...
                doRegister(
                  this.temp.pkPsndoc,
                  this.temp.registerUsername,
                  this.temp.registerPassword,
                  this.temp.captcha
                ).then((response) => {
                  if (response.success) {
                  } else {
