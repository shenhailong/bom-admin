  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: "pkPsndocShow", label: "业务员", width: "120" },
      ...
      top_btn: [{ name: "录入", code: "ADD", action: addProduct }],
      act_btn: [
        {
          name: "申请审核",
          code: "APPLYTECHNICS",
          when: { technicsState: [0] },
          action: applyProductToTechnics,
          width: 330,
        },
        {
          name: "修改",
  ...
  methods: {
    onButtonClick(param) {
      ...
      if (param.btnCode === "APPLYTECHNICS") {
        ...
        let applyParam = {
          pkProduct: rowData.pkProduct,
          pkPsndoc: rowData.pkPsndoc,
          technicsState: 20,
        };
        // .then((response) => {
          this.$confirm("是否申请?", "提示", {
            ...
            .then(() => {
              ...
              applyProductToTechnics(applyParam)
              this.queryListData()
              // location.reload();
            })
            .catch(() => {
