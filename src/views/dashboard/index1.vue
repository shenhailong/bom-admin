...
export default {
  ...
  methods: {
    handleSetMsgView(type) {
      if (type === '待办事项') {
		   //刷新list进行最新时间排序
		  this.todoUserTaskList.sort((a,b)=>{
			  let atimeString=a.creationtime;
			  let btimeString=b.creationtime;
        ...
        this.showTaskMsgList = this.todoUserTaskList
		   ...
		   //刷新list进行最新时间排序
		  this.claimUserTaskList.sort((a,b)=>{
		  			  let atimeString=a.creationtime;
		  			  let btimeString=b.creationtime;
		  			  atimeString=atimeString.replace(/-/g,'/');
		  			  btimeString=btimeString.replace(/-/g,'/');
        ...
        this.showTaskMsgList = this.claimUserTaskList
		   ...
		   //刷新list进行最新时间排序
		  this.overUserTaskList.sort((a,b)=>{
		  			  let atimeString=a.creationtime;
		  			  let btimeString=b.creationtime;
		  			  atimeString=atimeString.replace(/-/g,'/');
		  			  btimeString=btimeString.replace(/-/g,'/');
